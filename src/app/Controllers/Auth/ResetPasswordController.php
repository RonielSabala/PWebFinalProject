    ...
    public function handle(Template $template)
    {
        ...
        if ($_POST['action'] == "validate_code") {
            ...
            if (!isset($_SESSION['reset_password_code']) || time() > $_SESSION['reset_password_code_expiration_time']) {
                GenericUtils::showAlert("Código expirado. Vuelve a solicitarlo.", "danger",  "forgot_password.php");
                $template->apply();
            } elseif ($code == $_SESSION['reset_password_code']) {
                $_SESSION['is_code_valid'] = true;
                header('Location: reset_password.php');
            } else {
                GenericUtils::showAlert("Código incorrecto.", "danger", showReturn: false);
                $template->apply();
            }
        ...
        if (empty($_SESSION['is_code_valid'])) {
            GenericUtils::showAlert("Acceso no autorizado.", "danger", "forgot_password.php");
            $template->apply();
            exit;
        }
        ...
        if ($password !== $confirm_password) {
            GenericUtils::showAlert("Las contraseñas no coinciden.", "danger", showReturn: false);
            $template->apply();
            exit;
        }
        ...
        if (!$success_response) {
            GenericUtils::showAlert("Error al actualizar la contraseña. Intenta de nuevo.", "danger", showReturn: false);
            $template->apply();
            exit;
        }
